...
#            print(self.content)
    ...
    def addPackage(self,package, options= None):
        if options is None:
            self.document_data['packages'] = '\\usepackage{%s}' % package
        else:
            self.document_data['packages'] = '\\usepackage[%s]{%s}' % (options,package)

    def addPreamble(self,text):
        self.document_data['preamble'] = text
...
#        self.mainText = '%s\n%s' % (self.mainText,text)
    ...
    def addFig(self,fig,caption = '', width = r"0.8\textwidth",dpi = 200,label=''):
